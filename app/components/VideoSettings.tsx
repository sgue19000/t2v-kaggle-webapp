    ...
    <div className="rounded-3xl border border-line bg-panel p-4">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          ...
          className="text-xs text-mint disabled:opacity-50"
        >
          Reset
        </button>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Frames">
          <select
            ...
            onChange={(e) => patch({ numFrames: Number(e.target.value) })}
        ...
        <Field label="Steps">
          <select
            disabled={disabled}
            value={value.numInferenceSteps}
            onChange={(e) => patch({ numInferenceSteps: Number(e.target.value) })}
            className="w-full rounded-[0.9rem] border border-line bg-ink px-3 py-2 text-zinc-100 outline-none"
          >
            <option value={20}>20 faster</option>
            <option value={25}>25 default</option>
            <option value={30}>30 sharper</option>
          </select>
        </Field>
        <Field label="Guidance">
          <input
            ...
            onChange={(e) => patch({ guidanceScale: Number(e.target.value) })}
            className="w-full rounded-[0.9rem] border border-line bg-ink px-3 py-2 text-zinc-100 outline-none"
          />
        </Field>
        <Field label="Seed">
      ...
      <textarea
        ...
        className="mt-1 min-h-[4.5rem] w-full resize-y rounded-[0.9rem] border border-line bg-ink px-3 py-2 text-zinc-100 outline-none"
      />
      <p className="mt-3 text-[11px] leading-relaxed text-zinc-500">
        Stay on 16 frames / 256px on a free Kaggle P100. Higher settings often hit VRAM limits or the 12-hour session timeout.
      </p>
    </div>
  );
}
